...
public class ASN1 {
  ...
  /**
   * This class is used internal to the ASN.1 decoding functions. After decoding there will be one
   * of these for each item in the original in the encoded byte array
   */
  ...
  // 1.2.840.10045.3.1.7
  public static final byte[] Oid_secp256r1 = new byte[] {
      0x06,
      0x08,
      0x2A,
      (byte) 0x86,
  ...
  // 1.3.132.0.34
  public static final byte[] Oid_secp384r1 = new byte[] {
      0x06,
      0x05,
      0x2B,
      (byte) 0x81,
  ...
  // 1.3.132.0.35
  public static final byte[] Oid_secp521r1 = new byte[] {
      0x06,
      0x05,
      0x2B,
      (byte) 0x81,
  ...
  // 1.2.840.10045.2.1
  public static final byte[] oid_ecPublicKey = new byte[] {
      0x06,
      0x07,
      0x2a,
      (byte) 0x86,
  ...
  // 1.3.101.112
  public static final byte[] Oid_Ed25519 = new byte[] {
      0x6,
      0x3,
      0x2b,
      101,
  ...
  // 1.3.101.113
  public static final byte[] Oid_Ed448 = new byte[] {
      0x6,
      0x3,
      0x2b,
      101,
  ...
  public static final byte[] Oid_rsaEncryption = new byte[] {
      0x06,
      0x09,
      0x2A,
      (byte) 0x86,
  ...
  /**
   * Encode a subject public key info structure from an OID and the data bytes for the key This
   * function assumes that we are encoding an EC Public key.d
   *
  ...
  public static byte[] EncodeSubjectPublicKeyInfo(
      byte[] algorithm,
      byte[] keyBytes) throws CoseException {
    // SPKI ::= SEQUENCE {
    // algorithm SEQUENCE {
   ...
   */
  public static byte[] EncodeEcPrivateKey(
      byte[] oid,
      byte[] keyBytes,
      byte[] spki) throws CoseException {
  ...
  /*
   * Decode an object which is supposed to be a SubjectPublicKeyInfo strucuture and check that the
   * right set of fields are in the right place
   *
  ...
  /**
   * Decode an array of bytes which is supposed to be an ASN.1 encoded structure. This code does the
   * decoding w/o any reference to a schema for what is being decoded so it returns type and value
   * pairs rather than converting the values to the correct underlying data type.
   *
   * One oddity that needs to be observed is that Object Identifiers do not have the type and length
   * removed from them. This is because we do a byte wise comparison and started doing the entire
   * item rather than just the value portion.
   *
   ...
   */
  public static TagValue DecodeCompound(int offset, byte[] encoding)
      throws CoseException {
      ...
      if ((tag & 0x20) != 0) {
        ...
        if (tag == 6) {
          result.add(
              new TagValue(
                  tag,
                  Arrays.copyOfRange(encoding, offset, offset + l[1] + l[0] + 1)));
        } else {
          result.add(
              new TagValue(
                  tag,
   ...
   * @throws CoseException - ASN.1 encoding errors
   */
  public static TagValue DecodeSimple(int offset, byte[] encoding)
      throws CoseException {
    ArrayList<TagValue> result = new ArrayList<TagValue>();
    int retTag = encoding[offset];
   ...
   * @throws CoseException - ASN.1 encoding errors
   */
  public static byte[] EncodePKCS8(
      byte[] algorithm,
      byte[] keyBytes,
      byte[] spki) throws CoseException {
    // PKCS#8 ::= SEQUENCE {
   ...
   * @throws CoseException - ASN.1 encoding errors
   */
  public static ArrayList<TagValue> DecodePKCS8Structure(byte[] encodedData)
      throws CoseException {
   ...
   * @throws CoseException - ASN.1 encoding errors
   */
  public static ArrayList<TagValue> DecodePKCS8RSA(ArrayList<TagValue> pkcs8)
      throws CoseException {
    ...
    if (pkdl.size() < 8 || pkdl.size() > 11)
      throw new CoseException(
          "Invalid RSAPrivateKey");
   ...
   * @throws CoseException - ASN.1 encoding errors
   */
  public static ArrayList<TagValue> DecodePKCS8EC(ArrayList<TagValue> pkcs8)
      throws CoseException {
    // Decode the contents of the octet string PrivateKey
    ...
    if (pkdl.size() > 2) {
      if ((pkdl.get(2).tag & 0xff) != 0xA0) {
        ...
        if (pkdl.size() == 4 && (pkdl.get(3).tag & 0xff) != 0xa1)
          throw new CoseException("Invalid ECPrivateKey");
      }
    }
  ...
  public static byte[] EncodeSignature(byte[] r, byte[] s)
      throws CoseException {
  ...
  public static byte[] AlgorithmIdentifier(byte[] oid, byte[] params)
      throws CoseException {
    ArrayList<byte[]> xxx = new ArrayList<byte[]>();
    xxx.add(oid);
    if (params != null) {
