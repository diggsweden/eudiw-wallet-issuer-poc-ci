 ...
 * @author jimsch
 */
public class Recipient extends COSEObject {

  COSEKey privateKey;
  private COSEKey senderKey;
  byte[] rgbEncrypted;
   ...
   * @deprecated In COSE 0.9.1, use SetKey(COSEKey)
   */
  @Deprecated
  public void SetKey(CBORObject key) throws CoseException {
    privateKey = new COSEKey(key);
  }
  ...
  public void SetSenderKey(CBORObject key) throws CoseException {
    senderKey = new COSEKey(key);
  }

  public void SetSenderKey(COSEKey key) {
    senderKey = key;
  }
  ...
  private void ECDH_GenEphemeral() throws CoseException {
    COSEKey secretKey = COSEKey.generateKey(privateKey.get(KeyKeys.EC2_Curve));

    // pack into EPK header
    CBORObject epk = secretKey.PublicKey().AsCBOR();
    addAttribute(HeaderKeys.ECDH_EPK, epk, UNPROTECTED);

    // apply as senderKey
    senderKey = secretKey;
  }

  private byte[] ECDH_GenSecret(COSEKey key) throws CoseException {
    COSEKey epk;
    if (senderKey != null) {
      epk = key;
      key = senderKey;
