...
/**
 * The Sign1COSEObject class implements the COSE_Sign1 object. This provides for a single signature
 * where the signer is normally implicitly known. {@link SignCOSEObject} need to be used in those
 * cases where more than one signature is needed on a content. There is no way to convert a signed
 * message between the two formats.
 * <p>
 * Create a Sign1COSEObject object for a new message, when processing an existing message use
 * COSEObject.DecodeFromBytes to create the Sign1COSEObject object.
 * <p>
 * There are examples of using this class: <br>
 * Simple Example
 *
 * @author jimsch
 */
public class Sign1COSEObject extends SignCommon {

  byte[] rgbSignature;

  /**
   * Create a signed message object for which there will be a leading tag and the content will be
   * included.
   */
  public Sign1COSEObject() {
    this(true, true);
  }
   ...
   */
  public Sign1COSEObject(boolean emitTag) {
    this(emitTag, true);
  }
   ...
   * @since COSE 0.9.1
   */
  public Sign1COSEObject(boolean emitTag, boolean emitContent) {
    this.emitTag = emitTag;
    this.contextString = "Signature1";
    this.emitContent = emitContent;
    this.coseObjectTag = COSEObjectTag.Sign1;
  }
   ...
   */
  public void sign(COSEKey key) throws CoseException {
    if (rgbContent == null)
  ...
  public boolean validate(COSEKey cnKey) throws CoseException {
    CBORObject obj = CBORObject.NewArray();
    obj.Add(contextString);
    if (objProtected.size() > 0)
      obj.Add(rgbProtected);
   ...
   */
  @Override
  protected void DecodeFromCBORObject(CBORObject messageObject)
      throws CoseException {
    if (messageObject.size() != 4)
      throw new CoseException(
          "Invalid Sign1 structure");
    ...
    // Experimental attempt to detect CBOR Tagged data, and in such case, preserve the CBOR tags in
    // the signed data.
    if (messageObject.get(2).HasTag(EInteger.FromInt32(24))) {
      rgbContent = messageObject.get(2).EncodeToBytes();
    } else {
      if (messageObject.get(2).getType() == CBORType.ByteString)
        rgbContent =
            messageObject.get(2).GetByteString();
    ...
    if (messageObject.get(3).getType() == CBORType.ByteString)
      rgbSignature =
          messageObject.get(3).GetByteString();
    else
      throw new CoseException("Invalid Sign1 structure");
  }
